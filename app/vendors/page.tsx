...
const riskColor = (level: "LOW" | "MEDIUM" | "HIGH") =>
  level === "LOW"
    ...
    : level === "MEDIUM"
    ? "text-black/70 border-white/20 bg-white/5"
    : "text-black/50 border-white/10 bg-white/5";
    ...
    <main className="min-h-screen pt-24 pb-16 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-10">
          ...
          <h1 className="font-serif text-4xl lg:text-5xl font-bold text-black mt-2 mb-3">Discover Vendors</h1>
          <p className="text-black/45 text-lg">Browse and analyze vendors with AI-powered risk intelligence.</p>
        </div>
          ...
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-3 rounded-xl bg-white/5 border border-white/8 text-black placeholder:text-black/30 text-sm outline-none focus:border-gold/50 transition-colors"
              />
            </div>
            ...
            <select
              ...
              className="px-4 py-3 rounded-xl bg-white/5 border border-white/8 text-black text-sm outline-none cursor-pointer"
            >
                ...
                <button
                  ...
                  className={`px-4 py-3 rounded-xl text-sm font-medium transition-all ${
                    riskFilter === r
                      ...
                      : "glass border border-white/8 text-black/60 hover:text-black"
                  }`}
                >
                  {r}
                </button>
            ...
            <div className="flex gap-1 glass rounded-xl p-1 border border-white/5">
              <button onClick={() => setView("grid")} className={`p-2 rounded-lg transition-all ${view === "grid" ? "bg-gold/20 text-gold" : "text-black/40"}`}>
              ...
              <button onClick={() => setView("list")} className={`p-2 rounded-lg transition-all ${view === "list" ? "bg-gold/20 text-gold" : "text-black/40"}`}>
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16"/></svg>
              </button>
            </div>
          </div>

          <p className="text-black/30 text-xs mt-4">{filtered.length} vendor{filtered.length !== 1 ? "s" : ""} found</p>
        </div>
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-24">
            ...
            <p className="text-black/40 text-lg">No vendors match your filters.</p>
          </div>
        ) : view === "grid" ? (
                ...
                <div className="group glass border border-white/5 hover:border-gold/30 rounded-2xl p-6 transition-all duration-300 hover:glow-gold-sm hover:-translate-y-0.5 h-full">
                  ...
                  <div className="flex items-start gap-4 mb-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-semibold text-black text-sm truncate group-hover:text-gold transition-colors">{v.name}</h3>
                      <p className="text-black/40 text-xs mt-0.5">{v.category}</p>
                    </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 mb-4">
                    <div className="text-center glass rounded-lg py-2">
                      ...
                      <p className="text-black/30 text-[10px]">Reliability</p>
                    </div>
                    <div className="text-center glass rounded-lg py-2">
                      <p className="text-gold font-bold text-sm">{v.yearsInBusiness}y</p>
                      <p className="text-black/30 text-[10px]">Est.</p>
                    </div>
                    <div className="text-center glass rounded-lg py-2">
                      <p className="text-gold font-bold text-sm">{v.legalDisputes}</p>
                      <p className="text-black/30 text-[10px]">Disputes</p>
                    </div>
                  </div>

                  {/* Location */}
                  <p className="text-black/35 text-xs mb-4">📍 {v.location}</p>

                  {/* Compliance */}
                  <div className="flex gap-1.5 flex-wrap">
                    {v.compliance.slice(0, 2).map((c) => (
                      <span key={c} className="px-2 py-0.5 rounded-md text-[10px] glass text-black/40 border border-white/5">{c}</span>
                    ))}
                    {v.compliance.length > 2 && <span className="text-[10px] text-black/25">+{v.compliance.length - 2}</span>}
                    {v.compliance.length === 0 && <span className="text-[10px] text-black/20 italic">No certs</span>}
                  </div>
                </div>
              </Link>
            ))}
            ...
            {filtered.map((v) => (
              <Link key={v.id} href={`/vendors/${v.id}`}>
                <div className="group glass border border-white/5 hover:border-gold/30 rounded-xl px-6 py-4 transition-all duration-200 hover:glow-gold-sm flex items-center gap-6">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-black text-sm group-hover:text-gold transition-colors">{v.name}</p>
                    <p className="text-black/35 text-xs mt-0.5">{v.category} · {v.location}</p>
                  </div>
                  <div className="hidden md:flex items-center gap-8 text-sm">
                    ...
                    <div className="text-center">
                      <p className="text-black/60">{v.yearsInBusiness} yrs</p>
                      <p className="text-black/30 text-[10px]">Est.</p>
                    </div>
                    <div className="text-center">
                      <p className="text-black/60">{v.employees}</p>
                      <p className="text-black/30 text-[10px]">Employees</p>
                    </div>
                  </div>
                  <span className={`px-3 py-1 rounded-full text-xs font-bold border shrink-0 ${riskColor(v.riskLevel)}`}>{v.riskLevel}</span>
                  <svg className="w-4 h-4 text-black/20 group-hover:text-gold transition-colors shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
