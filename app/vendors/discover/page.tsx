...
// ── Shared helpers (copied exactly from compare page) ─────────────────────────
...
const riskColor = (color: RiskColor) =>
  color === "GREEN" ? "text-gold" : color === "YELLOW" ? "text-white/60" : "text-white/40";
...
function RatingStars({ rating }: { rating: number }) {
  return (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((s) => (
        <svg key={s} className={`w-3 h-3 ${s <= Math.round(rating) ? "text-gold" : "text-white/15"}`} fill="currentColor" viewBox="0 0 20 20">
    ...
    <div>
      ...
      <div className="relative">
        <select value={value} onChange={(e) => onChange(e.target.value)} disabled={disabled}
          className="w-full appearance-none px-4 py-3 rounded-xl bg-white/[0.06] border border-white/10 text-white text-sm focus:outline-none focus:border-gold/50 transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
          {placeholder && <option value="" className="bg-navy-900 text-white/40">{placeholder}</option>}
          {children}
        </select>
...
function VendorCard({ vendor, selected, onToggle, selectionFull, selectionIndex }: {
  ...
  return (
    <div className={`relative glass border rounded-2xl p-5 transition-all ${
      ...
      : canSelect ? "border-white/[0.08] hover:border-white/20"
      ...
      <div className="flex items-start gap-4">
        <button onClick={onToggle} disabled={!canSelect}
          className={`shrink-0 mt-0.5 w-5 h-5 rounded border-2 flex items-center justify-center transition-all ${
            ...
            : canSelect ? "border-white/25 hover:border-gold/60 bg-transparent"
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2 mb-1">
            <h3 className="text-white font-semibold text-sm leading-snug">{vendor.name}</h3>
            <StatusBadge status={vendor.business_status} />
          </div>
          <p className="text-white/40 text-xs mb-2 leading-relaxed">{vendor.address}</p>
          {vendor.rating && <RatingStars rating={vendor.rating} />}
          <p className="text-white/55 text-xs mt-2.5 leading-relaxed italic">{vendor.summary}</p>
          <div className="mt-3">
      ...
      <button onClick={() => setExpanded((p) => !p)}
        className="mt-4 w-full flex items-center justify-between text-white/30 text-xs hover:text-white/60 transition-colors pt-4 border-t border-white/[0.06]">
        ...
        <div className="mt-4 space-y-2.5">
          {vendor.phone && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-white/55 text-xs">{vendor.phone}</span>
            </div>
          )}
          {vendor.website && (
            <div className="flex items-center gap-2">
          ...
          {vendor.opening_hours && vendor.opening_hours.length > 0 && (
            <div className="mt-2">
              <p className="text-white/25 text-xs uppercase tracking-widest mb-1.5">Hours</p>
              {vendor.opening_hours.slice(0, 4).map((h) => (
                <p key={h} className="text-white/40 text-xs">{h}</p>
              ))}
            </div>
          )}
          <a href={vendor.maps_url} target="_blank" rel="noopener noreferrer"
            className="inline-flex items-center gap-1.5 mt-1 text-xs text-white/40 hover:text-gold transition-colors">
...
// ── Audit / Compare Panel (exact compare page layout) ─────────────────────────
...
function AuditPanel({ report, vendors, onClose }: {
  ...
  // ── Metric definitions — identical axes to compare page ──────────────────
  const metrics: Array<{
    label: string;
    desc: string;
  ...
  // Count wins per vendor (same logic as compare page)
  ...
  // Recommendation — lowest risk_score, ties broken by reliability
  ...
  // For the risk-color label (same text as compare page)
    ...
    <div className="mt-10 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <h2 className="font-serif text-4xl lg:text-5xl font-bold text-white mt-1">Compare Vendors</h2>
          <p className="text-white/45 text-lg mt-1">AI-researched side-by-side analysis with live data.</p>
        </div>
        <button onClick={onClose}
          className="flex items-center gap-2 px-4 py-2 glass border border-white/10 rounded-xl text-xs text-white/50 hover:text-white/80 hover:border-white/20 transition-all shrink-0">
          <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {/* Overall score strip */}
      <div className="glass border border-white/5 rounded-2xl px-6 py-4 flex items-center gap-6 flex-wrap">
        <div>
          <p className="text-white/30 text-xs uppercase tracking-widest">Portfolio Safety Score</p>
          <p className={`font-serif text-3xl font-bold ${report.overall_score >= 70 ? "text-emerald-400" : report.overall_score >= 40 ? "text-yellow-400" : "text-red-400"}`}>
            {report.overall_score}<span className="text-white/20 text-lg font-normal"> / 100</span>
          </p>
        </div>
        <div className="flex-1 h-2 rounded-full bg-white/5 min-w-[120px]">
        ...
        <p className="text-white/20 text-xs w-full sm:w-auto">
          Generated {new Date(report.generated_at).toLocaleString()} · ID {report.request_id.slice(0, 8)}
        </p>
      </div>

      {/* ── Vendor header cards — exact compare page style ─────────────────── */}
      <div className={`grid gap-5 ${cols}`}>
        {vendors.map((v) => {
          ...
          return (
            <div key={v.place_id}
              className={`glass rounded-2xl p-5 text-center transition-all ${isBest ? "border-gold-bright glow-gold" : "border border-white/5"}`}>
      ...
      {/* ── Comparison table — exact compare page structure ────────────────── */}
      <div className="glass border border-white/5 rounded-2xl overflow-hidden">

        {/* Table header */}
        <div className={`grid border-b border-white/5 ${tableCols}`}>
          <div className="px-6 py-4">
            <p className="text-white/30 text-xs uppercase tracking-widest">Metric</p>
          </div>
          {vendors.map((v) => (
            <div key={v.place_id} className="px-6 py-4 border-l border-white/5 text-center">
        ...
        {metrics.map((m, idx) => (
          <div key={m.label}
            className={`grid border-b border-white/5 last:border-b-0 ${tableCols} ${idx % 2 === 1 ? "bg-white/[0.015]" : ""}`}>
            <div className="px-6 py-5">
              <p className="text-white/70 text-sm font-medium">{m.label}</p>
              <p className="text-white/30 text-xs mt-0.5">{m.desc}</p>
            </div>
            {vendors.map((v) => (
              <div key={v.place_id} className="px-6 py-5 border-l border-white/5">
        ...
        {/* Certifications row — exact compare page style */}
        <div className={`grid border-t border-white/5 ${tableCols}`}>
          <div className="px-6 py-5">
        ...
        <div className={`grid border-t border-white/5 ${tableCols}`}>
          <div className="px-6 py-5">
            <p className="text-white/70 text-sm font-medium">AI Assessment</p>
            <p className="text-white/30 text-xs mt-0.5">Research findings</p>
          </div>
          {vendors.map((v) => {
            const sc = scorecard(v);
            ...
            return (
              <div key={v.place_id} className="px-6 py-5 border-l border-white/5">
                ...
                <p className="text-white/45 text-xs leading-relaxed">{sc?.reason ?? "—"}</p>
                <p className="text-white/30 text-xs leading-relaxed mt-1.5">
        ...
        <div className={`grid border-t border-white/5 ${tableCols}`}>
          <div className="px-6 py-5">
            <p className="text-white/70 text-sm font-medium">Google Data</p>
            <p className="text-white/30 text-xs mt-0.5">Rating · Reviews · Status</p>
          </div>
          {vendors.map((v) => (
            <div key={v.place_id} className="px-6 py-5 border-l border-white/5 space-y-2">
              {v.rating != null && (
                <div className="flex items-center gap-2">
                  <div className="flex items-center gap-0.5">
                    {[1,2,3,4,5].map((s) => (
                      <svg key={s} className={`w-3 h-3 ${s <= Math.round(v.rating!) ? "text-gold" : "text-white/15"}`} fill="currentColor" viewBox="0 0 20 20">
                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                      </svg>
                    ))}
                  </div>
                  <span className="text-white/60 text-xs font-medium">{v.rating.toFixed(1)}</span>
                  {v.total_ratings != null && <span className="text-white/25 text-xs">({v.total_ratings.toLocaleString()})</span>}
                </div>
              )}
              <StatusBadge status={v.business_status} />
              {v.phone && <p className="text-white/40 text-xs">{v.phone}</p>}
            </div>
          ))}
        </div>
      </div>

      {/* ── Backup vendors for RED-rated ───────────────────────────────────── */}
      {redCards.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-4">
            <div className="w-1 h-4 rounded-full bg-gold" />
            <h3 className="text-white font-semibold text-sm">Suggested Backup Vendors</h3>
          </div>
            ...
            {redCards.map((sc) => (
              <div key={sc.name} className="glass border border-red-500/15 rounded-2xl p-5">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <p className="text-white/70 text-sm font-medium">Alternatives for <span className="text-white">{sc.name}</span></p>
                </div>
                {sc.backups.length > 0 ? (
                  ...
                  <p className="text-white/30 text-sm italic">No backup vendors suggested.</p>
                )}
              </div>
            ))}
          </div>
      ...
      {/* ── AI Recommendation — exact compare page style ───────────────────── */}
...
export default function VendorDiscoverPage() {
  ...
  // Pre-fill location from onboarding address saved in localStorage
  useEffect(() => {
    try {
      const saved = localStorage.getItem("vendoriq_business_address");
      if (saved) {
    ...
    <main className="min-h-screen pt-24 pb-32 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <h1 className="font-serif text-5xl lg:text-6xl font-bold text-white mt-2 mb-4">Vendor Discovery</h1>
        ...
        <form onSubmit={handleSearch} className="glass-gold border-gold rounded-3xl p-8 mb-10">
          <div className="mb-6">
            <label className="block text-white/50 text-xs font-semibold uppercase tracking-widest mb-2">Vendor Category</label>
            <input type="text" value={category} onChange={(e) => setCategory(e.target.value)}
              placeholder="e.g. Electronics supplier"
              className="w-full px-4 py-3 rounded-xl bg-white/[0.06] border border-white/10 text-white placeholder-white/25 text-sm focus:outline-none focus:border-gold/50 transition-colors"
              disabled={loading} />
            <div className="flex flex-wrap gap-1.5 mt-2.5">
              {CATEGORY_PRESETS.map((p) => (
                <button key={p} type="button" onClick={() => setCategory(p)}
                  className={`px-2.5 py-1 rounded-full text-xs font-medium border transition-all ${
                    category === p ? "bg-gold/15 border-gold/40 text-gold" : "bg-white/[0.04] border-white/8 text-white/40 hover:text-white/70 hover:border-white/20"
                  }`}>{p}</button>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            <SelectBox label="Country" value={countryCode} onChange={handleCountryChange} disabled={loading}>
              {allCountries.map((c) => <option key={c.isoCode} value={c.isoCode} className="bg-navy-900">{c.flag} {c.name}</option>)}
            </SelectBox>
            <SelectBox label="State / Province" value={stateCode} onChange={handleStateChange}
              ...
              {states.map((s) => <option key={s.isoCode} value={s.isoCode} className="bg-navy-900">{s.name}</option>)}
            </SelectBox>
          </div>

          <div>
            <div className="flex items-center gap-3 mb-3">
              <span className="text-white/50 text-xs font-semibold uppercase tracking-widest">Location by</span>
              <div className="inline-flex glass border border-white/8 rounded-full p-0.5">
                {(["city", "zipcode"] as const).map((mode) => (
                  <button key={mode} type="button"
                    onClick={() => { setLocationMode(mode); setCityName(""); setZipcode(""); }}
                    className={`px-4 py-1.5 rounded-full text-xs font-semibold transition-all ${locationMode === mode ? "bg-gold text-navy-900" : "text-white/40 hover:text-white/70"}`}>
            ...
            {locationMode === "city" ? (
              <SelectBox label="City (Optional)" value={cityName} onChange={setCityName}
                ...
                {cities.map((c) => <option key={`${c.name}-${c.stateCode}`} value={c.name} className="bg-navy-900">{c.name}</option>)}
              </SelectBox>
            ) : (
              <div>
                <label className="block text-white/50 text-xs font-semibold uppercase tracking-widest mb-2">ZIP / Postal Code (Optional)</label>
                <input type="text" value={zipcode} onChange={(e) => setZipcode(e.target.value)}
                  placeholder="e.g. 94102" maxLength={10} disabled={loading || !stateCode}
                  className="w-full sm:w-52 px-4 py-3 rounded-xl bg-white/[0.06] border border-white/10 text-white placeholder-white/25 text-sm focus:outline-none focus:border-gold/50 transition-colors disabled:opacity-40" />
                {!stateCode && <p className="text-white/25 text-xs mt-1.5">Select a state first</p>}
              </div>
            )}
          </div>
          ...
          {stateCode && (
            <div className="mt-4 flex items-center gap-2">
              ...
              <span className="text-white/40 text-xs">Searching near: <span className="text-gold/80 font-medium">{locationString}</span></span>
            </div>
          )}
        ...
        {/* Discovery loading */}
        {loading && (
          <div className="glass border border-white/5 rounded-3xl p-10 text-center mb-8">
            <div className="flex items-center justify-center gap-4 mb-5 flex-wrap">
              {["Searching Google Places", "Fetching vendor details", "AI summarizing"].map((step, i) => (
                <div key={step} className="flex items-center gap-2">
                  ...
                  {i < 2 && <span className="text-white/15 mx-2">→</span>}
                </div>
              ))}
            </div>
            <p className="text-white/25 text-sm">Mistral AI is researching vendors near <span className="text-white/50">{locationString}</span>…</p>
          </div>
        )}
        ...
        {result && (
          <div>
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-white/35 text-sm mt-1">
                  <span className="text-gold/80">{result.query}</span> near <span className="text-white/60">{result.location}</span>
                  <span className="text-white/20 mx-2">·</span>
                  <span className="text-white/25">{new Date(result.generated_at).toLocaleTimeString()}</span>
                </p>
              </div>
            </div>

            <div className="flex items-center gap-2 mb-6 px-4 py-3 glass border border-white/[0.06] rounded-xl">
              <svg className="w-4 h-4 text-gold/60 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <p className="text-white/40 text-sm">
                Check up to <span className="text-gold font-semibold">{MAX_SELECTION} vendors</span> then click <span className="text-gold font-semibold">Compare &amp; Audit</span> for a scored side-by-side analysis with Risk Score, Reliability, Financial Stability, Review Validation, Years in Business, Legal Disputes, and Compliance.
              </p>
            </div>

            {result.vendors.length === 0 ? (
              <div className="glass border border-white/5 rounded-2xl p-10 text-center">
                <p className="text-white/30 text-sm">No vendors found. Try a different category or location.</p>
              </div>
            ) : (
            ...
            {/* Audit loading */}
            {auditLoading && (
              <div className="glass border border-white/5 rounded-3xl p-10 text-center mt-8">
                <div className="flex items-center justify-center gap-4 mb-5 flex-wrap">
                ...
                <p className="text-white/25 text-sm">Mistral agents are auditing <span className="text-white/50">{selectedVendors.map((v) => v.name).join(", ")}</span>...</p>
              </div>
            )}
        ...
        {!result && !loading && !error && (
          <div className="glass border border-white/5 rounded-2xl p-8 text-center">
            ...
            <p className="text-white/40 text-sm">Select a country and state to discover real vendors. City is optional.</p>
            <p className="text-white/20 text-xs mt-2">Powered by Mistral AI · Google Places · Tavily</p>
          </div>
        )}
      </div>
      ...
      {selectedIds.length > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 w-full max-w-2xl px-4">
          <div className="glass-gold border-gold rounded-2xl px-5 py-4 shadow-[0_8px_40px_rgba(198,167,94,0.2)] flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-white font-semibold text-sm">{selectedIds.length} / {MAX_SELECTION} vendors selected</p>
              <p className="text-white/40 text-xs truncate">{selectedVendors.map((v) => v.name).join(" · ")}</p>
            </div>
            <button onClick={() => { setSelectedIds([]); setAuditReport(null); }}
              className="text-white/40 hover:text-white/70 transition-colors shrink-0 p-1.5">
